...
def Bonus_perspective_warping(img1, img2, img3):
    ...
    img1 = np.uint8(
        Laplacian_Pyramid_Blending_with_mask(
            out, img1, np.float32(out_mask), num_levels=4))
...
def Cylindrical_warping(img1, img2, img3):
    ...
    out = cv2.warpAffine(
        imcyl2[0],
        M[:-1], (im1.shape[1], im1.shape[0]),
        dst=im1.copy(),
        borderMode=cv2.BORDER_TRANSPARENT)
    out_mask = cv2.warpAffine(
        imcyl2[1],
        M[:-1], (im1.shape[1], im1.shape[0]),
        dst=np.zeros_like(im1),
        borderMode=cv2.BORDER_TRANSPARENT) / 255
...
def Bonus_cylindrical_warping(img1, img2, img3):
    ...
    im1 = np.uint8(
        Laplacian_Pyramid_Blending_with_mask(
            out, im1, np.float32(out_mask / 255), num_levels=3))
