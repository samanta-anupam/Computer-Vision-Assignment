...
def calculate_mask(overlay):
    ...
    mask = np.where(graph_cut[superpixels], 255, 0).astype('uint8')
    return mask
...
if __name__ == '__main__':
    ...
    mask = calculate_mask(img_marking)
    disp_img = mark_boundaries(img, superpixels, color=(0, 255, 255))
    disp_img = (disp_img * 255.0).astype('u1')
    cv2.imshow('image', disp_img)
    # ======================================== #

    sample = cv2.imread('example_output.png', cv2.IMREAD_GRAYSCALE)
    # print(RMSD(sample, mask))
    # read video file
